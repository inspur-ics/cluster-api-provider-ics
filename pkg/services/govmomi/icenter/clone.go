...
import (
	...
	"github.com/inspur-ics/cluster-api-provider-ics/pkg/services/govmomi/template"
)
...
func Clone(ctx *context.VMContext, bootstrapData []byte) error {
	...
	if ctx.ICSVM.Spec.CloneMode == "" || ctx.ICSVM.Spec.CloneMode == infrav1.LinkedClone {
		ctx.Logger.Info("linked clone requested")
		// If the name of a snapshot was not provided then find the template's
		// current snapshot.
		if snapshotName := ctx.ICSVM.Spec.Snapshot; snapshotName == "" {
			ctx.Logger.Info("searching for current snapshot")
			var vm mo.VirtualMachine
			if err := tpl.Properties(ctx, tpl.Reference(), []string{"snapshot"}, &vm); err != nil {
				return errors.Wrapf(err, "error getting snapshot information for template %s", ctx.ICSVM.Spec.Template)
			}
			if vm.Snapshot != nil {
	...
	ctx.ICSVM.Status.CloneMode = infrav1.FullClone
	if snapshotRef != nil {
		// Record the actual type of clone mode used as well as the name of
		// the snapshot (if not the current snapshot).
		ctx.ICSVM.Status.CloneMode = infrav1.LinkedClone
		ctx.ICSVM.Status.Snapshot = snapshotRef.Value
	...
	numCPUs := ctx.ICSVM.Spec.NumCPUs
	if numCPUs < 2 {
		numCPUs = 2
	}
	numCoresPerSocket := ctx.ICSVM.Spec.NumCoresPerSocket
	if numCoresPerSocket == 0 {
		numCoresPerSocket = numCPUs
	}
	memMiB := ctx.ICSVM.Spec.MemoryMiB
	if memMiB == 0 {
		memMiB = 2048
	}
...
func getDiskSpec(
	...
	disk.CapacityInKB = int64(ctx.ICSVM.Spec.DiskGiB) * 1024 * 1024
...
func getNetworkSpecs(
	...
	key := int32(-100)
	for i := range ctx.ICSVM.Spec.Network.Devices {
		netSpec := &ctx.ICSVM.Spec.Network.Devices[i]
