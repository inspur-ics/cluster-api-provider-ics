...
// +kubebuilder:rbac:groups=infrastructure.cluster.x-k8s.io,resources=icsclusters,verbs=get;list;watch;patch
// +kubebuilder:rbac:groups=infrastructure.cluster.x-k8s.io,resources=icsmachinetemplates,verbs=get;list;watch
...
func (r Reconciler) Reconcile(ctx *context.ClusterContext) (reconcile.Result, error) {
	...
	if !clustermodule.IsClusterCompatible(ctx) {
		conditions.MarkFalse(ctx.ICSCluster, infrav1.ClusterModulesAvailableCondition, infrav1.ICenterVersionIncompatibleReason, clusterv1.ConditionSeverityInfo,
			"iCenter API version %s is not compatible with cluster modules", ctx.ICSCluster.Status.ICenterVersion)
		ctx.Logger.Info("cluster is not compatible for anti affinity",
			"api version", ctx.ICSCluster.Status.ICenterVersion)
	...
	for _, mod := range ctx.ICSCluster.Spec.ClusterModules {
		...
		if obj, ok := objectMap[curr]; !ok {
			...
			// append the module and object info to the ICSCluster object
	...
	ctx.ICSCluster.Spec.ClusterModules = clusterModuleSpecs
	...
	case len(modErrs) > 0:
		...
		conditions.MarkFalse(ctx.ICSCluster, infrav1.ClusterModulesAvailableCondition, infrav1.ClusterModuleSetupFailedReason,
			clusterv1.ConditionSeverityWarning, generateClusterModuleErrorMessage(modErrs))
	case len(modErrs) == 0 && len(clusterModuleSpecs) > 0:
		conditions.MarkTrue(ctx.ICSCluster, infrav1.ClusterModulesAvailableCondition)
	default:
		conditions.Delete(ctx.ICSCluster, infrav1.ClusterModulesAvailableCondition)
	}
	return reconcile.Result{}, err
}
...
func (r Reconciler) toAffinityInput(obj client.Object) []reconcile.Request {
	...
	if err := r.Client.Get(r, client.ObjectKey{
		...
		r.Logger.Error(err, "failed to get ICSCluster object",
			"namespace", cluster.Namespace, "name", cluster.Spec.InfrastructureRef.Name)
		return nil
	}

	return []reconcile.Request{
		{NamespacedName: client.ObjectKeyFromObject(icsCluster)},
	}
}
...
func (r Reconciler) fetchMachineOwnerObjects(ctx *context.ClusterContext) (map[string]clustermodule.Wrapper, error) {
	...
	name, ok := ctx.ICSCluster.GetLabels()[clusterv1.ClusterLabelName]
	if !ok {
	...
	if err := r.Client.List(
		ctx, kcpList,
		client.InNamespace(ctx.ICSCluster.GetNamespace()),
