...
import (
	...
	"github.com/inspur-ics/cluster-api-provider-ics/api/v1alpha4"

	"github.com/onsi/gomega"
	"github.com/pkg/errors"
)

var unmarshalWarnAsFatal = []v1alpha4.UnmarshalINIOptionFunc{v1alpha4.WarnAsFatal}
...
type codecTestCase struct {
	...
	expectedError    error
	unmarshalOptions []v1alpha4.UnmarshalINIOptionFunc
}
...
`,
			configObj: v1alpha4.CPIConfig{
				Global: v1alpha4.CPIGlobalConfig{
					Username:    "user",
					Password:    "password",
					Datacenters: "us-west",
					ClusterID:   "cluster-namespace/cluster-name",
				},
				ICenter: map[string]v1alpha4.CPIICenterConfig{
					"0.0.0.0": {},
				},
				Workspace: v1alpha4.CPIWorkspaceConfig{
					Server:     "0.0.0.0",
					Datacenter: "us-west",
					Folder:     "kubernetes",
					Datastore:  "default",
		...
		{
			testName: "Username and password in iCenter section",
			iniString: `[Global]
insecure-flag = true
port = "443"
datacenters = "us-west"
...
`,
		...
		{
			testName: "Multiple iCenters using global CA cert",
			iniString: `[Global]
secret-name = "vccreds"
secret-namespace = "kube-system"
...
`,
			configObj: v1alpha4.CPIConfig{
				...
				ProviderConfig: v1alpha4.CPIProviderConfig{
					Cloud: &v1alpha4.CPICloudConfig{
...
func TestUnmarshalINI(t *testing.T) {
	...
	testcases := []codecTestCase{
		...
		{
			testName: "NetBIOS style AD username and password in iCenter section",
			iniString: `
		[Global]
		port = "443"
		insecure-flag = true
	...
	for _, tc := range testCases {
		tc := tc
		t.Run(tc.testName, func(t *testing.T) {
			var actualConfig v1alpha4.CPIConfig
