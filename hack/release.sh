...
# This script is used build CAPICS's container images.
...
readonly BASE_IMAGE_REPO=gcr.io/cluster-api-provider-ics

# Release images
readonly CAPICS_MANAGER_IMAGE_RELEASE=${BASE_IMAGE_REPO}/release/manager
readonly CAPICS_MANIFESTS_IMAGE_RELEASE=${BASE_IMAGE_REPO}/release/manifests

# PR images
readonly CAPICS_MANAGER_IMAGE_PR=${BASE_IMAGE_REPO}/pr/manager
readonly CAPICS_MANIFESTS_IMAGE_PR=${BASE_IMAGE_REPO}/pr/manifests

# CI images
readonly CAPICS_MANAGER_IMAGE_CI=${BASE_IMAGE_REPO}/ci/manager
readonly CAPICS_MANIFESTS_IMAGE_CI=${BASE_IMAGE_REPO}/ci/manifests
...
USAGE="
usage: ${0} [FLAGS]
  Builds and optionally pushes new images for Cluster API Provider ICS (CAPICS)
...
function build_images() {
  case "${BUILD_RELEASE_TYPE}" in
    ci)
      ...
      MANAGER_IMAGE_NAME=${CAPICS_MANAGER_IMAGE_CI}
      MANIFESTS_IMAGE_NAME=${CAPICS_MANIFESTS_IMAGE_CI}
      ;;
    pr)
      # A PR build
      MANAGER_IMAGE_NAME=${CAPICS_MANAGER_IMAGE_PR}
      MANIFESTS_IMAGE_NAME=${CAPICS_MANIFESTS_IMAGE_PR}
      ;;
    release)
      # On an annotated tag
      MANAGER_IMAGE_NAME=${CAPICS_MANAGER_IMAGE_RELEASE}
      MANIFESTS_IMAGE_NAME=${CAPICS_MANIFESTS_IMAGE_RELEASE}
      ;;
    *)
      # A local dev build
  ...
  docker build \
    ...
    --build-arg "CAPICS_MANAGER_IMAGE=${MANAGER_IMAGE_NAME}:${VERSION}" \
    .
  if [ "${LATEST}" ]; then
